...
def clean_lens_json(lens_output: dict, labeled_dataframe: pd.DataFrame):
    ...
    for paper in lens_output["data"]:
        ...
        paper_object["mesh"] = paper.get("mesh_terms", "") and \
            [mesh_term["mesh_id"]
                for mesh_term in paper["mesh_terms"] if mesh_term.get("mesh_id", "")]
...
if __name__ == "__main__":
    ...
    cleaned_json_dict = clean_lens_json(json_dict, labeled_dataframe)
    
    with open("cleaned_lens_output.json", "x") as cleaned_json:
        json.dump(cleaned_json_dict, cleaned_json)
        cleaned_json.close()
